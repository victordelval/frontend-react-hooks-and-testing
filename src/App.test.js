...
describe('<App />', () => {
  ...
  it('should render one router <Route /> to path "/"', () => {
    expect(wrapper.findWhere(route => route.prop('path') === '/')).toHaveLength(1)
  })

  it('should render one router <Route /> to path "/schedules-by-professor"', () => {
    expect(wrapper.findWhere(route => route.prop('path') === '/schedules-by-professor')).toHaveLength(1)
  })

  it('should render one router <Route /> to path "/schedules-by-subject"', () => {
    expect(wrapper.findWhere(route => route.prop('path') === '/schedules-by-subject')).toHaveLength(1)
  })
  ...
  it('should render <Professor /> when navigating to path "/schedules-by-professor"', () => {
    const component = mount(<MemoryRouter initialEntries={['/schedules-by-professor']}><App/></MemoryRouter>);
